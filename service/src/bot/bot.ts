...
export class ChatGPTBot {
  ...
  async getGPTMessage(talkerName: string, text: string): Promise<string> {
    let response = await chatgpt(talkerName, text);
    if (response.code != 200) {
      return response.msg;
    }
    // TODO web那边的接口会返回conversionid和messageid，这里要保存起来
    DBUtils.addAssistantMessage(talkerName, response.text, response.id, response.conversationId);
